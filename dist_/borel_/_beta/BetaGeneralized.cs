...
using System.Linq;
using System.Text;
namespace nilnul.stat.dist_.borel_._beta
{
		...
		:nilnul.prob.DistributionI
	{
		...
		public Generalized(double a, double b, double min, double max)
		{
			if (a <= 0 || b <= 0)
			{
				throw new Exception();
		...
		public double inverseCdf(double prob)
		{
			...
			return new nilnul.num.real.func_.binary_.beta._spanned.integrand.incomplete.Regularized(a, b, min, max).inverse(prob);

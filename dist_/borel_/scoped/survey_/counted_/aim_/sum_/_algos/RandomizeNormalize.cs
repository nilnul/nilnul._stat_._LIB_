...
namespace nilnul.prob.dist._continuum._realize._constrained._boundSum._algos
{
	/// <summary>
	/// 
	/// </summary>
	/// 
	public  class RandomizeThenNormalize
	{
		...
		static public int[] Exe_ceilingByDistribution(int countOfItems_positive, int sumTarget_nonNeg, int bound_nonNeg) {
			...
			while (distributed>epsilon)
			{
				...
				for (int i = 0; i < tails.Length; i++)
				{
					if (tails[i]!=0)
					{
						dict.Add(i, new _prob.InDouble( tails[i]/sum));
					}
				}



				var cdfPacked = Cdf.Packed.Create( new dist._finite.intDouble.Pmf.Packed(dict));

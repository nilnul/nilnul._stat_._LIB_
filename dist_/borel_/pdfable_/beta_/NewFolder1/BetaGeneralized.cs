...
namespace nilnul.prob.doubleProb.doubleSample.cdf.contains
{
		...
		:DistributionA
	{
		...
		public override Prob probability(double x)
		{
			if (x <= min)
			{
				return Prob.Zero;
			...
			return new Prob( nilnul.math.calculus.BetaFunction.Incomplete(a, b, (x - min) / span));
			;
		}

		public override double inverse(Prob prob)
		{
			return new nilnul.math.calculus.IncompleteBetaFunctionSpanned(a, b, min, max).inverse(prob._val);
		}
	}
}
